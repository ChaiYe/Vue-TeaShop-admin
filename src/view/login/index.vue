<template>
  <div class="login-container">
    ...
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <Verify @success="verifyInfo('success')" @error="verifyInfo('error')" :type="2"></Verify>
  ...
  methods: {
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true;
          /*this.$http.post(this.HOST+"/sys/login",this.loginForm).then(
              (response)=> {
          ...
          this.$store.dispatch('Login', this.loginForm).then(() => {
            this.loading = false;
            console.log("login/index.vue下的" + this.redirect);
